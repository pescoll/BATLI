...
# Set the Dataset you are working with

dataset = f'dataset_1_dotA'

# Load the data
df = pd.read_csv(dataset_1_dotA_path)
...
# # Function to calculate duplication compared to previous timepoint
# def calculate_duplication(group):
#     group['duplication'] = (group['legio_area_[um²]-_sum_per_cell_'] >= double_factor * group['legio_area_[um²]-_sum_per_cell_'].shift()).astype(int)
#     return group
...
def calculate_duplication(group):
    ...
    shifted_series = group['bacteria_area'].shift(1)
    group['duplication'] = (group['bacteria_area'] >= double_factor * shifted_series.expanding().mean()).astype(int)
    return group
...
plot_path = f"./vacuole_growth_script/bar_graph_{dataset}_{timestamp}.png"
...
percentage_results_path = f"./vacuole_growth_script/percentage_results_{dataset}_{timestamp}.csv"
